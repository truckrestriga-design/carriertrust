...
type TextPack = {
  ...
  searching: string;
  noMatchingCompanies: string;
  existingCompaniesHint: string;
  vatLabel: string;
  companyFallback: string;
  exactVatMatchFound: string;
  similarCompanyName: string;
  vatAlreadyExistsMustSelect: string;
  ...
  loginOrRegisterToClaim: string;
  createAccountToReview: string;
};
...
const TEXT: Record<Lang, TextPack> = {
  en: {
    ...
    vatAlreadyExistsMustSelect:
      "A company with this VAT already exists. You must select it to continue.",
  ...
  de: {
    ...
    claimSubtitle:
      "Melden Sie sich an oder registrieren Sie sich, um Ihr ausgewähltes Unternehmen zu beanspruchen.",
    ...
    existingCompaniesHint:
      "Wenn Ihr Unternehmen bereits bei CarrierTrust existiert, wählen Sie es unten aus.",
    vatLabel: "USt-IdNr.",
    ...
    vatAlreadyExistsMustSelect:
      "Ein Unternehmen mit dieser USt-IdNr. existiert bereits. Sie müssen es auswählen, um fortzufahren.",
    ...
    pleaseAcceptPolicies:
      "Bitte akzeptieren Sie die Bedingungen, die Datenschutzrichtlinie und die Bewertungsrichtlinie.",
  ...
  ru: {
    ...
    claimSubtitle:
      "Войдите или зарегистрируйтесь, чтобы подтвердить права на выбранную компанию.",
    ...
    existingCompaniesHint:
      "Если ваша компания уже есть в CarrierTrust, выберите её ниже.",
    vatLabel: "VAT",
    companyFallback: "Компания",
    exactVatMatchFound: "Найдено точное совпадение по VAT",
    similarCompanyName: "Похожее название компании",
    vatAlreadyExistsMustSelect:
      "Компания с таким VAT уже существует. Чтобы продолжить, нужно выбрать её.",
    ...
    pleaseAcceptPolicies:
      "Пожалуйста, примите Условия, Политику конфиденциальности и Политику отзывов.",
  ...
  fr: {
    ...
    claimSubtitle:
      "Connectez-vous ou créez un compte pour revendiquer l’entreprise sélectionnée.",
    ...
    existingCompaniesHint:
      "Si votre entreprise existe déjà sur CarrierTrust, sélectionnez-la ci-dessous.",
    vatLabel: "TVA",
    ...
    vatAlreadyExistsMustSelect:
      "Une entreprise avec ce numéro de TVA existe déjà. Vous devez la sélectionner pour continuer.",
    ...
    pleaseAcceptPolicies:
      "Veuillez accepter les Conditions, la Politique de confidentialité et la Politique des avis.",
  ...
  es: {
    ...
    claimSubtitle:
      "Inicia sesión o regístrate para reclamar tu empresa seleccionada.",
    ...
    passwordResetEmailSent:
      "Si existe una cuenta con este email, se ha enviado un enlace para restablecer la contraseña.",
    passwordUpdated:
    ...
    existingCompaniesHint:
      "Si tu empresa ya existe en CarrierTrust, selecciónala abajo.",
    vatLabel: "VAT",
    ...
    vatAlreadyExistsMustSelect:
      "Ya existe una empresa con este VAT. Debes seleccionarla para continuar.",
    ...
    pleaseAcceptPolicies:
      "Acepta los Términos, la Política de privacidad y la Política de reseñas.",
  ...
  it: {
    ...
    claimSubtitle:
      "Accedi o registrati per rivendicare l’azienda selezionata.",
    ...
    existingCompaniesHint:
      "Se la tua azienda esiste già su CarrierTrust, selezionala qui sotto.",
    vatLabel: "VAT",
    companyFallback: "Azienda",
    exactVatMatchFound: "Trovata corrispondenza esatta del VAT",
    similarCompanyName: "Nome azienda simile",
    vatAlreadyExistsMustSelect:
      "Esiste già un’azienda con questo VAT. Devi selezionarla per continuare.",
    ...
    pleaseAcceptPolicies:
      "Accetta i Termini, l’Informativa sulla privacy e la Policy recensioni.",
...
function detectCountryFromVat(vatRaw: string): string {
...
export default function AuthPage() {
  ...
  const [msg, setMsg] = useState<keyof TextPack | string | null>(null);
  ...
  const [claimContextCompanyId, setClaimContextCompanyId] = useState<string>("");
  const [claimContextLoaded, setClaimContextLoaded] = useState(false);
  ...
  const exactVatMatch = useMemo(() => {
    if (!normalizedCompanyVat) return null;
    return (
      companyMatches.find(
        (c) => String(c.vat_uid || "").trim().toUpperCase() === normalizedCompanyVat
      ) || null
    );
  }, [companyMatches, normalizedCompanyVat]);

  const hasClaimContext = Boolean(claimContextCompanyId);
  const vatDetectedCountry = useMemo(
    () => detectCountryFromVat(normalizedCompanyVat),
  ...
  async function ensureProfile(
    ...
    if (need) {
      const { error: upErr } = await supabase.from("profiles").upsert(patch);
      if (upErr) throw new Error(upErr.message);
    }
  }

  async function searchCompanies(nameRaw: string, vatRaw: string) {
    ...
    try {
      const byVatMap = new Map<string, CompanyMatch>();
      const byNameMap = new Map<string, CompanyMatch>();
      ...
      if (!stillExists) {
        const exact = merged.find(
          (c) => String(c.vat_uid || "").trim().toUpperCase() === vat
        );
  ...
  async function loadClaimContextCompany(companyId: string) {
    try {
      const { data, error } = await supabase
        .from("companies")
        .select("id, name, vat_uid, country")
  ...
  }, [companyName, companyVat, mode, hasClaimContext]);
  ...
  }, [companyVat, mode, hasClaimContext, selectedCompanyId, countryManual]);

  async function syncCompanyAccess() {
    const res = await supabase.functions.invoke("sync-company-access", {
      body: {},
    });

    const data: any = (res as any)?.data;
    const fnError: any = (res as any)?.error;
  ...
  const input =
    "w-full rounded-2xl border border-slate-200 bg-white/85 px-4 py-3 text-slate-900 shadow-sm outline-none backdrop-blur placeholder:text-slate-400 focus:border-slate-300";
  const readonlyInput =
    "w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 shadow-sm outline-none";
  const selectInput =
    "w-full rounded-2xl border border-slate-200 bg-white/85 px-4 py-3 text-slate-900 shadow-sm outline-none backdrop-blur focus:border-slate-300";
  ...
  if (!claimContextLoaded) {
    return (
      <main className="min-h-screen text-slate-900">
        ...
        <div className="relative flex min-h-screen items-center justify-center px-4 pt-32 pb-10">
            ...
            <div className="mt-6 space-y-4">
              {mode === "register" && (
                <>
                  ...
                  <div>
                    <div className="mb-2 text-sm font-semibold text-slate-800">{t.country}</div>

                    {hasClaimContext ? (
                      <input className={readonlyInput} value={companyCountry} readOnly />
                    ) : vatDetectedCountry && !selectedCompanyId ? (
                      <input className={readonlyInput} value={companyCountry} readOnly />
                    ) : (
                      <select
                    ...
                    {!companyCountry ? (
                      <div className="mt-2 text-xs text-slate-500">{t.countryHelp}</div>
                    ) : null}
                  </div>
                  ...
                  {!hasClaimContext &&
                    (searchingCompanies ||
                      ...
                      normalizedCompanyVat) && (
                      <div className={card}>
                        <div className="font-semibold text-slate-900">{t.existingCompanies}</div>

                        {searchingCompanies ? (
                          <div className="mt-2 text-slate-500">{t.searching}</div>
                        ) : companyMatches.length === 0 ? (
                          <div className="mt-2 text-slate-500">{t.noMatchingCompanies}</div>
                        ) : (
                          <>
                            <div className="mt-2 text-slate-600">
                              {t.existingCompaniesHint}
                            </div>
                            ...
                            <div className="mt-3 space-y-2">
                              {companyMatches.map((c) => {
                                ...
                                const isExactVat =
                                  normalizedCompanyVat && vat === normalizedCompanyVat;
                                  ...
                                  >
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <div className="mt-1 text-xs text-slate-500">
                                        {c.vat_uid
                                          ? `${t.vatLabel}: ${String(c.vat_uid).toUpperCase()}`
                                          : `${t.vatLabel}: —`}
                                        {c.country ? ` • ${c.country}` : ""}
                                      </div>
                                      ...
                                      {isExactVat ? (
                                        <div className="mt-1 text-xs font-semibold text-slate-900">
                                          {t.exactVatMatchFound}
                                        </div>
                                      ) : (
                                        <div className="mt-1 text-xs text-slate-500">
                                          {t.similarCompanyName}
                                        </div>
                                      )}
                                    </div>
                                  </label>
              ...
              {mode === "register" && (
                <label className="flex items-start gap-3 text-sm text-slate-600">
                  ...
                  <span className="leading-5">
                    {t.agreePrefix}{" "}
                    <a className="underline underline-offset-4" href="/terms" target="_blank">
                      {t.terms}
                    </a>
                    ,{" "}
                    <a className="underline underline-offset-4" href="/privacy" target="_blank">
                      {t.privacyPolicy}
                    </a>{" "}
                    and{" "}
                    <a
...
{msg && (
  <div className="rounded-2xl border border-slate-200 bg-white/80 p-3 text-sm text-slate-700 shadow-sm">
    {resolveMsg(msg)}
  </div>
)}
