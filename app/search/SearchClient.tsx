...
export default function SearchPage() {
  ...
  const [items, setItems] = useState<SearchCompany[]>([]);
  ...
  const searchSchema = useMemo(() => {
    ...
    return schema;
  }, [q, items, t.pageTitle, t.companyFallback]);
  ...
  useEffect(() => {
    async function loadBanners() {
      ...
      const { data } = await supabase
        ...
        .eq("is_active", true)
        .in("placement", ["search_left", "search_right"])
      ...
      const left = rows
        .filter((item: any) => item.placement === "search_left")
        .map((item: any) => ({
          id: item.id,
      ...
      const right = rows
        .filter((item: any) => item.placement === "search_right")
        .map((item: any) => ({
          id: item.id,
          image: item.image_url,
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/60 bg-white/70 p-6 shadow-[0_25px_80px_rgba(15,23,42,0.08)] backdrop-blur-xl md:p-8">
              ...
              <div className="relative">
                ...
                <p className="mt-4 text-sm leading-7 text-slate-600 md:text-[15px]">
                  {t.queryLabel}: <strong className="text-slate-900">{q || "—"}</strong>
                </p>

                <div className="mt-8">
                  <div className="group relative">
                    <div className="absolute -inset-1 rounded-2xl bg-gradient-to-r from-emerald-400 to-teal-400 opacity-20 blur transition duration-500 group-hover:opacity-30" />
                    <div className="relative flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-2 shadow-xl shadow-slate-200/40 md:flex-row md:items-center">
                ...
                <div className="mt-8 rounded-[1.5rem] border border-slate-200/70 bg-white/80 p-4 shadow-sm md:p-5">
                  {loading ? (
                    <div className="rounded-2xl border border-slate-200 bg-white px-5 py-6 text-slate-500">
                      {t.loading}
                    </div>
                  ) : items.length === 0 ? (
                    <div className="rounded-2xl border border-slate-200 bg-white px-5 py-6 text-slate-500">
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="hidden h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-slate-400 sm:flex">
          ...
          <RotatingBanner side="right" banners={rightBanners} onAddClick={openOrder} t={t} />
        </div>
            ...
            <div className="relative w-full max-w-4xl scale-[0.97] origin-center rounded-[2rem] border border-white/50 bg-white/92 shadow-[0_40px_120px_rgba(15,23,42,0.25)]">
              ...
              <div className="p-4 md:p-4">
                      ...
                      {selectedPlanData && (
                        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-2.5">
                          ...
                          <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[13px] text-slate-700">
                            ...
                            <p>
                              <span className="font-semibold">{t.bankBic}:</span>{" "}
                              {companyBankData.bic}
                            </p>
                            <p>
                ...
                <div className="mt-3 flex flex-col gap-2">
                  ...
                  >
                    {submitting ? t.sending : t.havePaid}
                  </button>
